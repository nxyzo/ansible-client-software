...
Ansible Role Wrapper for VLC Media Player Install and Uninstall (PSADT v4) from Jason Berger
...
To obtain the VLC Media Player installer, download the desired version from the official VLC download page at
https://download.videolan.org/pub/videolan/vlc/￼. Place the appropriate vlc-x.y.z-win32.exe or vlc-x.y.z-win32.msi, or the vlc-x.y.z-win64.exe or vlc-x.y.z-win64.msi file in the PSADT directory on your Ansible host (win-vlc/files/Files). The deployment role will locate and execute the installer from this path during the installation process.

(Optional) To disable automatic update checks and metadata network access, follow the [VLC Media Player Silent Install (How-To Guide)](https://silentinstallhq.com/vlc-media-player-silent-install-how-to-guide/) to generate the vlcrc file. Place this file in the win-vlc/files/SupportFiles directory.
